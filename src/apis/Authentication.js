...
export const loginUser = async (dispatch, user) => {
    ...
    try {
      const resp = await axios.post(process.env.REACT_APP_BASE_URL + '/auth/admin/login', user, {
        headers: {
  ...
  export const changePassword = async (dispatch, data) => {
    ...
    dispatch(actionStart())
    try {
      const resp = await axios.post(process.env.REACT_APP_BASE_URL + '/v1/auth/change-password', data, {
        headers: {
          'accept': 'application/json',
          'Content-type': 'application/json',
